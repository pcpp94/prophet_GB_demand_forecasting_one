...
from .inputs_default import initial_date, final_date_dict, forecast_date_dict, default_gdp_dic, default_pop_dictionary, default_cap_regressor
...
def make_daily_regressors_df(initial_date=initial_date):
    ...
    df['sat_reg'] = df['ds'].apply(lambda x: np.abs(
        x.timetuple().tm_yday - 182.6) if x.weekday() == 5 else 0)
    df['sun_reg'] = df['ds'].apply(lambda x: np.abs(
        x.timetuple().tm_yday - 182.6) if x.weekday() == 6 else 0)

    temp = ps.read_table(
        'ee_api_gold.weather_data_gb_hourly').to_pandas().reset_index(drop=True)
    ...
    future_temp = ps.read_table(
        'ee_api_gold.weather_data_gb_projected_hourly').to_pandas().reset_index(drop=True)
    ...
    average_temp_all = ps.read_table(
        'ee_api_gold.weather_data_gb_average_hourly').to_pandas().reset_index(drop=True)
    ...
    temp_2 = ps.read_table(
        'ee_api_gold.weather_data_gb_daily').to_pandas().reset_index(drop=True)
    temp_2 = temp_2[temp_2['date'] < temp_2['date'].max()]
    temp_2.columns = temp_2.columns.str.replace('gb_', '')
    temp_2 = temp_2.rename(columns={'date': 'ds'})
    temp_2 = temp_2.groupby('ds').agg(
        {'snow_depth': 'mean',
         'precipitation_mm': 'mean'}).reset_index()

    future_temp_2 = ps.read_table(
        'ee_api_gold.weather_data_gb_projected_daily').to_pandas().reset_index(drop=True)
    future_temp_2.columns = future_temp_2.columns.str.replace('gb_', '')
    future_temp_2 = future_temp_2.rename(columns={'date': 'ds'})
    future_temp_2 = future_temp_2.groupby('ds').agg(
        {'snow_depth': 'mean',
         'precipitation_mm': 'mean'}).reset_index()

    average_temp_all_2 = ps.read_table(
        'ee_api_gold.weather_data_gb_average_daily').to_pandas().reset_index(drop=True)
    average_temp_all_2.columns = average_temp_all_2.columns.str.replace(
        'gb_', '')
    average_temp_all_2 = average_temp_all_2.rename(columns={'date': 'ds'})
    average_temp_all_2 = average_temp_all_2.groupby('ds').agg(
        {'snow_depth': 'mean',
         'precipitation_mm': 'mean'}).reset_index()

    temp = temp.merge(temp_2, how='inner', left_on='ds', right_on='ds').reset_index(
        drop=True).merge(df, how='left', on='ds')
    future_temp = future_temp.merge(future_temp_2, how='inner', left_on='ds', right_on='ds').reset_index(
        drop=True).merge(df, how='left', on='ds')
    average_temp_all = average_temp_all.merge(average_temp_all_2, how='inner', left_on='ds', right_on='ds').reset_index(
        drop=True).merge(df, how='left', on='ds')
